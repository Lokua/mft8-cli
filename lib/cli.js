...
var logger = require('./logger').createLogger('cli');
var c = require('chalk');
...
function help() {
  var fpath = path.resolve(__dirname, 'usage.txt');
...
function list() {
  ...
  MidiUtil.getInputs().forEach(function (x, i) {
    return console.log('\t%s: %s', c.green(i), x);
  });

  console.log(c.yellow('\nOutputs'));
  MidiUtil.getOutputs().forEach(function (x, i) {
    return console.log('\t%s: %s', c.green(i), x);
  });
...
function setPortsInteractively() {
  var input = undefined,
      output = undefined,
      virtualIn = undefined,
      virtualOut = undefined;
  var correct = false;
  do {
    var inputs = MidiUtil.getInputs();
    var outputs = MidiUtil.getOutputs();

    console.log(c.yellow('---\nChoose the input port of your MIDI Fighter Twister\n'));
    inputs.forEach(function (input, number) {
      return console.log('%s: %s', c.green(number), input);
    });
    input = prompt('\nEnter number: ');

    console.log(c.yellow('---\nChoose the output port of your MIDI Fighter Twister\n'));
    outputs.forEach(function (output, number) {
      return console.log('%s: %s', c.green(number), output);
    });
    output = prompt('\nEnter number: ');
    ...
    console.log('\n%s: %s\n%s: %s\n%s: %s\n%s: %s\n', c.green(input), inputs[input], c.green(output), outputs[output], c.green(virtualIn), inputs[virtualIn], c.green(virtualOut), outputs[virtualOut]);

    // TODO: use y/n lib from awesome-node list?
    correct = prompt(c.yellow('Are these correct? (y/n): ')) === 'y' ? 1 : 0;
  } while (!correct);

  return { input: input, output: output, virtualIn: virtualIn, virtualOut: virtualOut };
}
